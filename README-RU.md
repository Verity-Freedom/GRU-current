# GRU
**Global Rolling Updater** (GRU current) — это система автоматического обновления, набор спецификаций и общие принципы развёртывания. Известным проектом, использующим GRU, является [Tor Portable](https://github.com/Verity-Freedom/Tor-Portable).
...
2. Текущая версия программы может быть указана через специальный файл (для примера назовём его VERSION) внутри самого архива, в её интерфейсе, в свойствах бинарных файлов, в директории с архивами. Рекомендуется помещать его в директорию с архивами и в сами архивы, так как это даёт наибольшее количество информации как для глаз пользователя, так и для программы. Реализовано автообновление по принципу сравнения файлов VERSION внутри архива и в директории с архивами — его можно добавить в код любой программы, и оно свяжется с основным апдейтером. По этой причине в названии файла VERSION не должно быть пробелов: если powershell ещё нормально с ними работает, то с curl возникают дополнительные сложности.
...
4. Само обновление происходит по схеме: 1) перевод директории на текущую; 2) удаление всех файлов и директорий в папке с копированием пользовательских; 3) скачивание архива; 4) распаковка архива; 5) удаление архива и всех временных файлов. Реализации на Windows и Linux отличаются: Linux хранит исполняемый скрипт в оперативной памяти и делает возможным его горячее удаление, в отличие от Windows. Поэтому на Windows необходимо создание нескольких параллельно запускающихся скриптов.
...
## Особенности GRU
1. Универсальность. Не требуется установки никаких отдельных программ для соблюдения всех спецификаций. Принципы системы могут быть совместимы со множеством файловых хранилищ (проверены IPNS и Dropbox).
